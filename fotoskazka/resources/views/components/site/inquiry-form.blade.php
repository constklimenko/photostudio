...
<form method="POST" action="{{ route('inquiry.store') }}" {{ $attributes->merge(['class' => 'mt-8 space-y-5']) }}>
    ...
    <div>
        ...
        <input type="text" name="name" id="name" required
               class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-amber-500 focus:ring-amber-500">
    ...
    <div>
        ...
        <textarea name="message" id="message" rows="4"
                  class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-amber-500 focus:ring-amber-500"></textarea>
    </div>

    <div class="flex items-start gap-2">
        <input type="checkbox" name="agreed_to_terms" id="agreed_to_terms" required
               class="mt-1 rounded border border-gray-300 text-amber-600 focus:ring-amber-500">
