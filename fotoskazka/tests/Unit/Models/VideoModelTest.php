...
class VideoModelTest extends TestCase
{
    ...
    public function test_source_url_returns_proxy_stream_route_for_uploaded_file(): void
    {
        $video = new Video(['file_path' => 'videos/clip.mp4', 'url' => 'https://example.com/video']);
        $video->id = 42;

        $this->assertEquals(route('video.stream', 42), $video->source_url);
        $this->assertStringContainsString('/video/42/stream', $video->source_url);
    }
