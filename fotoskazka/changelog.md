...
## 2026-09-26 — Оптимизация загрузки данных главной страницы

### Цель

Привести `HomeController` в соответствие с текущей структурой главной (Hero с
кнопками услуг и категорий, социальные ссылки, «Избранные работы») и убрать
лишние запросы, не удаляя сохранённые блоки и бизнес-логику.

### Что было проверено

`HomeController` уже запрашивал только данные выводимых блоков: страницу `home`,
Hero, соцсети и «Избранные работы». Запросов отключённых секций («Наши услуги»,
отзывы, видео, блог, вопросы, «О студии», форма заявки) в контроллере не
осталось — соответствующие методы `HomeContentService` сохранены и не
вызываются. Найдены две проблемы:

1. **N+1 в кнопках Hero при глубоком дереве категорий.** Цепочка предков
   загружалась фиксированно (`with('parent.parent.parent')` и
   `with('category.parent.parent')`), а `catalogPath()` обходит её до корня.
   Глубина дерева не ограничена, поэтому начиная с 4-го уровня предки
   дочитывались лениво — по запросу на кнопку.
2. **Лишняя загрузка коллекции для фона Hero.** `heroImages()` загружал все
   фотографии альбома типа `homepage` вместе со всеми `media`, тогда как
   `x-site.hero` использует только первую фотографию.

### Реализация

- `app/Models/Category.php` — новый `Category::loadAncestorChains()`:
  пакетная загрузка цепочек предков коллекции категорий любой глубины, по
  одному запросу на уровень; каждой загруженной категории выставляется
  `parent`-связь, поэтому `catalogPath()` не делает дополнительных запросов.
  В выборку берутся только `id`, `parent_id`, `slug` — полей, нужных для пути.
- `app/Services/HomeContentService.php`:
  - `heroButtons()` — вместо фиксированного eager loading вызывается
    `Category::loadAncestorChains()` для категорий-кнопок и категорий услуг;
    URL по-прежнему строятся существующими `catalogPath()`, порядок не изменился;
    `with('category')` ограничен тремя колонками.
  - `heroImages()` — берётся первая фотография (по `sort_order`) альбома типа
    `homepage` и её `media`, вместо всех фотографий альбома.
- `app/Http/Controllers/HomeController.php`, `home.blade.php`, компоненты
  `x-site.hero`, `x-site.social-links`, `x-site.home.featured-works`,
  модели и методы отключённых блоков не изменялись.

### Итог по запросам главной

Осталось 10 запросов на полной главной (было 14 при дереве категорий глубиной
от 4 уровней): страница `home`; альбом и первая фотография с `media` для фона
Hero; категории-кнопки; услуги-кнопки; по одному запросу на уровень цепочки
предков; категории услуг; соцсети; страницы с `show_on_home`; избранные альбомы
с обложками. N+1 не осталось ни по числу кнопок, ни по глубине дерева.

### Тесты

- `tests/Feature/Http/Controllers/HomeHeroButtonsTest.php`:
  - `test_page_show_on_home_flag_does_not_create_hero_buttons` — флаг
    `Page.show_on_home` включает только «Избранные работы» и не создаёт кнопок
    Hero;
  - `test_hero_buttons_are_loaded_without_n_plus_one_in_deep_tree` — на дереве
    глубиной 4 уровня число запросов к `categories` не превышает 5 и не растёт
    при добавлении ещё 5 категорий и 5 услуг, полный путь кнопок строится
    верно. Тест падает на прежней реализации с фиксированным eager loading.
- `tests/Feature/Http/Controllers/HomeControllerTest.php`:
  - `test_home_hero_uses_first_photo_of_homepage_album` — фон Hero берётся из
    фотографии с минимальным `sort_order`, остальные фотографии альбома не
    попадают в разметку.
- Остальные сценарии (выбранная/невыбранная/неопубликованная категория и услуга,
  `show_on_home` у страниц, отсутствие данных отключённых секций, отсутствие
  кнопок Hero) покрыты существующими тестами.

### Наблюдения вне объёма задачи

- `ViewComposerServiceProvider` на каждом запросе собирает `socialLinks`,
  которые на главной больше не используются (там данные берутся из
  `HomeContentService::socialLinks()`); глобальный `serviceList` нужен
  модальному окну заявки в шапке и странице услуги. Провайдер не менялся —
  это влияет на все страницы, а не только на главную.
- Кнопка услуги под неопубликованной категорией ведёт на 404 (резолвер требует
  опубликованную цепочку). Поведение не менялось: кнопки отбираются по
  `is_published` самих сущностей, как и раньше.

### Проверка

- `php artisan test` — 1136 тестов, 2985 assertions, все проходят;
  `./vendor/bin/pint --test` — без замечаний.

## 2026-09-26 — Блок «Стоимость альбомов»: миниатюры вместо точек, зацикленный слайдер

### Цель

В слайдере фотографий услуги внутри блока «Стоимость альбомов»
(`x-site.graduation-pricing`) заменить точки навигации на три миниатюры:
текущая фотография по центру и две соседние (предыдущая и следующая) по краям.
Слайдер должен быть зациклен — переход с последней фотографии на первую и обратно.

### Реализация

- `resources/views/components/site/graduation-pricing.blade.php` — фотографии
  без медиа отфильтрованы один раз (`$serviceSlides`); каждый слайд получил
  `data-slide-thumb` с URL миниатюры (`Media::getThumbnailUrl()`), после слайдера
  выводится пустой контейнер `data-album-thumbs` — только если слайдов больше
  одного.
- `resources/js/app.js` — `dots` отключён, `infinite` включён всегда при двух и
  более слайдах; миниатюры собираются из `data-slide-thumb` при инициализации
  slick, синхронизируются по событиям `afterChange` и `setPosition`, индексы
  берутся по кругу (`slickCurrentSlide` с нормализацией по модулю). Клик по
  левой миниатюре — `slickPrev`, по правой — `slickNext`, средняя отмечена
  классом `is-active` и `aria-current`.
- `resources/css/app.css` — удалены `padding-bottom` у `.album-slider` и стили
  `.slick-dots`; добавлены `.album-slider-thumbs` (сетка из трёх колонок) и
  `.album-slider-thumb` (активная — золотая рамка, соседние приглушены).
- Для альбома с одной фотографией миниатюры и стрелки не выводятся: зацикливать
  нечего.

### Тесты

- `tests/Feature/Http/Controllers/GraduationPricingCategoryPageTest.php` —
  новые тесты: при двух фотографиях выводятся контейнер миниатюр и URL миниатюры
  (по одному `data-slide-thumb` на слайд), при одной фотографии контейнер
  миниатюр не выводится.

### Проверка

- Ручная проверка в headless Chrome на `/services/vypusknye-albomy`: миниатюры
  соответствуют соседям текущего слайда, переходы вперёд/назад зациклены
  (после последнего слайда — первый, после первого — последний).
- `php artisan test` — 1133 теста, 2969 assertions, все проходят; `./vendor/bin/pint --test` — без замечаний.

## 2026-09-26 — Категория «Выпускные альбомы»: описание, цена и «Что входит» не выводятся

### Цель

На странице корневой категории, помеченной флагом
`categories.is_graduation_albums = true`, не показывать:

- `description` (блок с текстом категории под шапкой);
- `price_from` вместе с `price_note`;
- `items` (блок «Что входит»).

Для таких категорий вся информация о составе и стоимости выводится отдельным
блоком «Стоимость альбомов» (`x-site.graduation-pricing`) по подкатегориям и
услугам, поэтому дублирование не нужно. Для обычных категорий вывод не
изменился.

### Реализация

- `resources/views/services/category.blade.php` — блоки «Описание», «Цена» и
  «Что входит» обёрнуты в `@if (! $category->is_graduation_albums)`; вложенность
  отступов выровнена. SEO-данные (`meta description`, JSON-LD `Product`) и
  `AggregateOffer` продолжают использовать `seo_description`/`description`/
  `price_from` — это метаданные, а не видимый контент.

### Тесты

- `tests/Feature/Http/Controllers/GraduationCategoryExtrasPageTest.php` —
  новый тест: на странице категории с флагом не выводятся описание, цена,
  примечание к цене, заголовок «Что входит» и подписи пунктов, при этом
  остаются SEO-описание, блок «Стоимость альбомов» с ценой услуги и форма
  заявки. Существующий тест
  `test_category_page_without_graduation_flag_keeps_standard_layout` и тесты
  обычной категории (`ServiceCatalogControllerTest`) подтверждают прежний вывод
  для категорий без флага.
...
При создании страницы в админке, заголовок которой транслитерируется в системный
slug (`home`, `services`, `portfolio`, `blog`, `video`), сохранение падало с ошибкой:

```
SQLSTATE[HY000]: General error: 1364 Field 'slug' doesn't have a default value
insert into `pages` (`title`, `menu_title`, ...) values (Портфолио, ...)
```

Поле `slug` отсутствовало в `INSERT`.

### Причина

В `PageForm` поле `slug` помечается `disabled()` для системных страниц, чтобы
администратор не мог переименовать служебный URL. В Filament `isDehydrated()` по
умолчанию равно `! isDisabled()` (`Filament\Schemas\Components\Concerns\HasState`),
поэтому отключённое поле полностью исключалось из дегидратации формы и не
попадало в запрос. Для несистемных slug поле оставалось активным, поэтому баг
проявлялся только при совпадении заголовка со служебным.

### Реализация

- `app/Filament/Resources/Pages/Schemas/PageForm.php` — в поле `slug` добавлен
  явный `->dehydrated()`: значение сохраняется всегда, при этом поле остаётся
  нередактируемым для системных страниц. Схема БД и модель не менялись.
- `tests/Feature/Filament/PageResourceTest.php` — два регрессионных теста,
  отправляющих форму через Livewire (`fillForm` + `create`). До фикса падали с
  `NOT NULL constraint failed: pages.slug`. Существующие тесты использовали
  только фабрики и не проверяли дегидратацию формы, поэтому баг не ловили.
...
Завершить исследовательскую часть D1.1 инструкцией для практических прототипов:
короткая матрица «что подготовить → что протестировать → что измерить → как
принять результат» для подэтапов D1.2 и D1.3. Решений о технологии не принимает
(принятие — на контрольной точке D1.5), код/БД/архитектура не изменялись.

### Реализация

- `docs/AR-PROTOTYPE-TEST-PLAN.md` (новый) — минимальный план эксперимента.
- Первые технологии на прототип: **MindAR v1.2.5** и **8th Wall Open Source
  (MIT, Image Targets)**; AR.js — B-план при провале критерия iOS.
- Состав стенда: 2–3 фото (контрольная «хорошая» + студийный портрет/групповой
  снимок), 5 отпечатков (мат 10×15 и 20×30, глянец 10×15), 2 устройства
  (средний Android Chrome + iPhone Safari), 3 режима света (искусственный,
  дневной, блик).
- Сценарии tracking: распознавание, удержание ≥5 сек, потеря/повторное
  распознавание, запуск/пауза видео, автозапуск muted на iOS.
- Метрики и критерии принятия — из roadmap D1.2/D1.3 без изменений
  (≥50% попыток, ≥2 устройств; видео на ≥1 паре «SDK × телефон» при
  искусственном и дневном свете).

### НЕ изменено

- Код Laravel, миграции, модели, конфигурация и структура БД — без изменений.
- `roadmap.md` / `architecture.md` / `database.md` — без изменений.
- Эксперимент D1.2–D1.3 не выполнялся (план создан, но прогон не начинался).

---

## 2026-09-20 — D1.1: документ требований к фотографии как Image Target

### Цель

Дополнить исследование D1.1 (`docs/AR-TECHNOLOGY-RESEARCH.md`) практическими
требованиями к фотографии, используемой как Image Target, и ответить на ключевой
вопрос подэтапа: можно ли использовать саму напечатанную фотографию как target
без специальной переработки. Документ исследовательский, решения не принимает,
код/БД/архитектура не изменялись.

### Реализация

- `docs/AR-TARGET-REQUIREMENTS.md` (новый) — свод требований по
  feature points, контрасту, повторяющимся/симметричным элементам, крупным
  однотонным областям, глянцевой/матовой печати, размеру, качеству печати,
  освещению, дистанции и углу наклона.
- Все численные требования разделены по нотации:
  `[ПОДТВ]` — подтверждено документацией технологии (например, сторона
  исходника ~1000 px у MindAR, DPI ≥300 для AR.js, преимущество матовой печати
  у Vuforia/NRSDK), `[ЭКСП]` — проверяется экспериментом на D1.2–D1.3,
  `[ОБЩ]` — общий вывод feature-based tracking.
- Вывод по ключевому вопросу: сама фотография может выступать target без
  переработки только при достаточном числе/распределении feature points
  (проверяется визуально в компиляторе до печати); для типовых
  школьных/студийных портретов высока вероятность необходимости отдельной
  подготовки (кроп, окантовка, локальный контраст, матовый носитель).
  Численные пороги в документации не зафиксированы — вынесены в
  экспериментальный набор D1.2–D1.3.

### НЕ изменено

- Код Laravel, миграции, модели, конфигурация и структура БД — без изменений.
- `roadmap.md` / `architecture.md` / `database.md` — без изменений (этап D1.5
  по-прежнему является точкой принятия решения; документ не фиксирует выбор SDK).
- Задачи D1.2–D1.3 не начинались (запрет на переход к реализации соблюдён).
...
Фотография в lightbox была слишком маленькой (`max-h-[46vh]`) на больших экранах.

### Реализация

- `resources/views/components/site/album-photos.blade.php` — у `#lightboxImage`
  на экранах `sm+` установлены `max-h-[calc(100vh-13rem)]` и
  `max-w-[calc(100vw-4rem)]`: изображение вписывается в окно почти на всю высоту,
  оставляя отступы под кнопки управления (сверху `pt-20`, снизу `pb-32`).
- На мобильных (`< sm`) размер сохранён прежним — `46vh`/`92vw`, чтобы оставалось
  место для панели комментариев.

## 2026-09-15 — Автогенерация alt для фотографий в альбоме

### Цель

Настроить осмысленный атрибут `alt` для `<img>` в сетке фотографий и lightbox.

### Формирование ALT

- Если у фото/медиа есть собственное название (`$photo->caption` или
  `$photo->media->title`): `"[Название фото] — [Название альбома] Фотосказка Уфа"`.
- Иначе по номеру в коллекции (`$loop->iteration`):
  `"Фото [Номер] — [Название альбома] | Профессиональная фотосъемка в Уфе"`.

### Реализация

- `resources/views/components/site/album-photos.blade.php`:
  - вычисление `$albumTitle` в верхнем `@php`-блоке;
  - внутри цикла `@foreach` через `@php` формируется `$photoAlt` по описанным
    правилам (используется `$photo->caption ?? $photo->media->title ?? ''`);
  - атрибут `alt="{{ $photoAlt }}"`;
  - lightbox берёт alt из текущей миниатюры сетки (`img.alt` в JS), поэтому
    получает тот же текст без дополнительных запросов.

### Примечания

- Новых запросов нет: связи `media` и `album` уже загружаются в
  `CabinetService::paginateAlbumPhotos()` (`with(['media', 'comments.user',
  'album.project'])`) и `PortfolioController::show()` (`with(['photos.media'])`).
- `{{ }}` автоматически экранирует строку через `e()`, кавычки не ломают HTML.
...
Реализовать комментарии непосредственно к `Photo` в галерее личного кабинета
(`cabinet.album`) на базе существующей polymorphic-модели `comments` (C2.8).
Новая таблица **не создавалась** — `commentable_type`/`commentable_id` уже
рассчитаны на `Photo` (связь `Photo::comments()` зарезервирована в C2.8).

Право видеть/оставлять комментарий к фото равно праву видеть это фото:
цепочка `CommentPolicy::create` → `PhotoPolicy::view` → `AlbumPolicy::view`.
Ролевая матрица не дублируется:

- `client` — фото из своих проектов (любые типы альбомов);
- `class_manager` — фото из `client`-альбомов своего проекта;
- `parent` — фото только из назначенных ему `client`-альбомов
  (теперь может комментировать — закрыто противоречие, зафиксированное в C2.8);
- `photographer` / `admin` — все фото.

### Безопасность (IDOR)

- ID фото берётся только из маршрута `/cabinet/photos/{photo}/comments`
  (implicit route model binding), **не из формы**; все обрабатываемые поля формы —
  только `body`;
- `StorePhotoCommentRequest::authorize()` → `$user->can('create',
  [Comment::class, $photo])` → `PhotoPolicy::view` — запрет до валидации,
  чужая фотография → `403`, комментарий не создаётся;
- тест подтверждает, что `photo_id` из тела формы игнорируется: комментарий
  создаётся только для фото из URL;
- несуществующее фото → `404` (модельная привязка).

### Реализация

- `POST /cabinet/photos/{photo}/comments` (`CommentController@storePhoto`,
  middleware `auth`) — создание комментария, редирект в галерею альбома с
  фрагментом `#photo-{id}` (lightbox открывается на нужном фото);
- `StorePhotoCommentRequest` — авторизация + валидация `body`:
  `required|string|max:2000`;
- `CabinetService::paginateAlbumPhotos()` — eager loading
  `media`, `comments.user`, `album.project` — без N+1;
- галерея (`cabinet/album.blade.php`): компонент `album-photos` получил флаг
  `:comments="true"` — бейдж с числом комментариев на каждом фото; комментарии
  в lightbox выведены как **аккордеон в нижней части экрана**: всегда
  отображается кнопка «Комментарии (N)», контент (автор, дата, текст, форма)
  раскрывается по клику и по умолчанию свёрнут (не занимает место);
  форма добавления — только если пользователь имеет право комментировать
  (один gate-проверка на страницу — доступ по альбому однороден);
- на публичных страницах (портфолио/услуги) комментарии не рендерятся:
  флаг `comments` по умолчанию `false`, N+1 исключён;
- вывод `body` — `{!! nl2br(e($body)) !!}` — XSS безопасно.

### Намеренно НЕ реализовано

- лайки, статусы фотографий, модерация в Filament (roadmap C2.9),
  уведомления — вне текущего плана;
- редактирование/удаление комментариев — вне плана (как и в C2.8).

### Тесты

- `tests/Feature/Http/Controllers/Cabinet/CabinetPhotoCommentsTest.php` (новый,
  26 тестов): гость → login; client — свои фото (успех + любые типы альбомов +
  IDOR чужого → 403 + `photo_id` из формы игнорируется); class_manager —
  client-альбом своего проекта (успех) / project-альбом и чужой проект (403);
  parent — назначенный альбом (успех) / неназначенный и не-client (403);
  photographer/admin — любое фото; без роли — 403; body required/string/max;
  несуществующее фото → 404; матрица доступа через Gate; рендеринг (автор,
  дата, форма, XSS, сортировка, изоляция комментариев проекта/фото, публичное
  портфолио без UI); N+1 ≤ 15 запросов.
...
Зафиксировать в roadmap новый крупный этап по подготовке и созданию собственного
веб-сервиса «оживающих фотографий» (WebAR): пользователь наводит камеру смартфона
на напечатанную фотографию и видит связанный с ней цифровой контент (прежде всего
видео), без обязательной установки отдельного мобильного приложения. На текущем
этапе реализация (миграции, модели, AR Resource, SDK) **не** выполняется — этап
закладывает исследования, прототипирование и контрольную точку принятия решения.

### Изменено

- **roadmap.md** — добавлен этап `# D1 — Собственный сервис «оживающих
  фотографий» (WebAR)` (между Этапом 7 и разделом «Текущий статус»):
  - цель и результат этапа;
  - десять последовательных подэтапов D1.1–D1.10, каждый с задачей,
    ожидаемым результатом и критерием готовности:
    D1.1 исследование технологий (WebAR, Image Tracking, варианты SDK,
    ограничения мобильных браузеров, лицензирование, требования к печатному
    маркеру, ограничения видео);
    D1.2 первый технический прототип (image target, открытие контента,
    реальный смартфон, напечатанное фото, стабильность tracking);
    D1.3 второй прототип (сценарий «оживающей фотографии», видео поверх/вместо
    фото, размеры/качество печати, освещение, UX без установки приложения);
    D1.4 архитектурное проектирование (предварительная структура
    `Project → Album → Photo → AR Target → AR Content → Video Media` — без
    фиксации; стабильный AR token, связь target↔фото и content↔видео,
    независимое долгосрочное хранение AR-видео, жизненный цикл клиентского
    альбома, контроль доступа, несколько AR-контентов на одно фото);
    D1.5 контрольная точка принятия решения (собственная реализация /
    коммерческий AR-сервис / гибрид — собственная реализация не обязательна);
    D1.6 MVP собственного AR-сервиса (target, видео, стабильная ссылка,
    публичная WebAR-страница, tracking, запуск видео, статистика, Filament);
    D1.7 хранение и доставка (роль Яндекс.Диска и S3, CDN, стоимость хранения
    и трафика, независимость от клиентских альбомов);
    D1.8 безопасность и эксплуатация (доступ, валидация файлов, лимиты,
    стабильные AR URL, отключение контента, логирование, аналитика);
    D1.9 тестирование (desktop / Android / iPhone / браузеры / распечатанные
    фото / размеры / освещение / расстояние / угол / повторное распознавание /
    видео);
    D1.10 интеграция с сайтом (модели/миграции, Filament UI, AR routes,
    Media/Video, хранение, замена статичного AR-тизера на демо после MVP);
  - зависимости, границы этапа («Что не входит в D1») и критерии завершения.

### НЕ изменено

- Структура БД — миграции и модели не создавались; предложенная цепочка
  сущностей D1.4 является предварительной и не фиксируется до контрольной
  точки D1.5.
- Существующий маркетинговый AR-тизер (`x-site.ar-teaser`, `pages.ar_teaser_*`)
  — не удалялся и не перерабатывался; его замена на реальное демо предусмотрена
  только в D1.10 после готовности MVP.
- `architecture.md` — сознательно не изменялся: архитектура AR-сервиса
  зафиксирована в roadmap как будущая работа и будет проработана на D1.4–D1.5
  (после прототипов), без преждевременной фиксации SDK или структуры данных.
- `database.md` — не изменялся.
- Конкретный AR SDK в документацию как обязательная зависимость не добавлялся.

### Проверка

- `git diff roadmap.md changelog.md` — изменения ограничены двумя документами;
- тестовый набор и Pint не затрагиваются (правки документации).
